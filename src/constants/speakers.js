...
export const speakers = {
  sergio:{
    ...
    description:'<span class="collapsible-header">Nuevas técnicas de hacking a través de inyección de código en recursos javascript que permiten obtener datos como tarjetas de crédito...</span><span class="collapsible-body"><span> contraseñas y demás información importante para el cliente según el contexto.<br/>¿Qué medidas de seguridad tomar a la hora de proteger los datos del lado del cliente?<br/><br/>Muchas veces la información llega a terceros maliciosos primero antes de llegar al host, recordemos el caso de los 380.000 usuarios afectados por ataque reciente a la British Airways, explicare como se realizo este ataque y veremos algunas formas de protegernos como usar formularios de terceros, escuchar todas las conexiones salientes del navegador por medio de javascript; tanto las xhr como fetch, cómo usar iframes para aumentar la seguridad en conexiones salientes. se mencionaran brevemente algunas herramientas de codigo abierto como xhook.</span></span>'
  },
  luis_alejandro:{
  ...
  irina:{
    ...
    description:'<div>So you heard you can run WebAssembly in the browser, but what does this look like IRL? Seeking the much talked about performance benefits of running WASM in the browser...</div><div><span id="text"> Compass (GUI for MongoDB) took the great leap forward and rewrote part of the application -- a schema parser responsible for document analysis -- in Rust. As it turns out, the rumours are true, I repeat the rumours are true: offloading parts of your application to Rust makes it fast, and in some of our cases, heaps fast.<br/><br/>In this talk I will cover converting a part of an codebase from JavaScript to Rust, bringing it to Electron, adding WASM to an existing React app, and deploying this whole lot to production. As Compass still talks to MongoDB via Node.js, I will also cover how we used Rust to improve performance without losing data integrity. This talk is about a success story, but also about a bunch of smol mishaps, fun woopses and lil dead ends we encountered and how we managed to overcome along the way.</span></div><button id="toggle">Read More...</button>'
  },
  melina:{
